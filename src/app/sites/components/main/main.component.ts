...
export class MainComponent implements OnInit {
  ...
  public nextLoading = 60;

  constructor(public routing: ActivatedRoute,private route: Router, private dbConn: DbconnectService, private messageService: MessageService) { }
  ...
  async ngOnInit(): Promise<void> {
    ...
    setInterval(()=>{this.nextLoading = 60; this.loadData()}, 60000);
  ...
  public incrementModerator(id){
    this.dbConn.incrementModerator(id);
      ...
      /*
    if(this.token == "EwroonStinky1920"){
      this.dbConn.incrementModerator(this.nowEdit);
      this.display = false;
      this.messageService.add({severity:'info', summary:'Sukces', detail:'Poprawnie potwierdzono!'});
    } else{
      this.display = false;
      this.messageService.add({severity:'error', summary:'Błąd', detail:'Niepoprawny token!'})
    }*/
  }
